---
title: "Kaggle Competition_Data605, House prices Final exam part#2"
...
```{r fetching dataset, message=FALSE, warning=FALSE}
...
```{r explore train data, message=FALSE, warning=FALSE}
dim(train)
dim(test)
library(knitr)
library(magrittr)
...
```{r remove Ids and label testing, message=FALSE, warning=FALSE}
...
```{r bind test train, message=FALSE, warning=FALSE}
test$SalePrice <- NA
...
```{r int vs char, message=FALSE, warning=FALSE}
...
```{r NA percentage, message=FALSE, warning=FALSE}
...
```{r missing per variable, message=FALSE, warning=FALSE, paged.print=FALSE}
NAcol <- which(colSums(is.na(house)) > 0)
sort(colSums(sapply(house[NAcol], is.na)), decreasing = TRUE)
cat('There are', length(NAcol), 'columns with missing values')
```

## Visualization step - Detailed EDA {.tabset}
...
```{r response distrbution, message=FALSE, warning=FALSE}
...
```{r normalizing distribution, message=FALSE, warning=FALSE}
...
```{r response summary stats, message=FALSE, warning=FALSE}
...
```{r correlation nums and chars, fig.height=8, fig.width=8, message=FALSE, warning=FALSE}
...
```{r Visualize 6 imp vars, fig.height=10, fig.width=10, message=FALSE, warning=FALSE}
...
```{r vis_overallqality vs saleprice, message=FALSE, warning=FALSE}
...
```{r abovegrad vs saleprice, message=FALSE, warning=FALSE}
...
```{r compare outliers, message=FALSE, warning=FALSE}
house[c(524, 1299), c('SalePrice', 'GrLivArea', 'OverallQual')]
```

## Exam questions {.tabset}

### Correlation bettween three vars

Derive a correlation matrix for any three quantitative variables in the dataset.

```{r correlation_mat, message=FALSE, warning=FALSE}
cor_se <- house_numVar %>% 
...
```{r confid_interval, message=FALSE, warning=FALSE}
...
```{r familywise_error, message=FALSE, warning=FALSE}
perc <- (1 - (1 - 0.05)^3)
...
```{r cor_invert, message=FALSE, warning=FALSE}
...
```{r multiplying pre vs cor, message=FALSE, warning=FALSE}
...
```{r LU_decomp, message=FALSE, warning=FALSE}
# LU decomposition 
...
```{r confirm the results, message=FALSE, warning=FALSE}
...
```{r histogram of right skewed var, message=FALSE, warning=FALSE}
...
```{r pdf, message=FALSE, warning=FALSE}
...
```{r compare to original, message=FALSE, warning=FALSE}
...
```{r 5th and 95th percentile, message=FALSE, warning=FALSE}
# simulated
(cdf_lo <- log(1 - .05)/-lam)
(cdf_up <- log(1 - .95)/-lam)
...
```{r empirical CI, message=FALSE, warning=FALSE}
...
```{r How many NAs, message=FALSE, warning=FALSE}
NAcol <- which(colSums(is.na(house)) > 0)
sort(colSums(sapply(house[NAcol], is.na)), decreasing = TRUE)
cat('There are', length(NAcol), 'columns with missing values')
```
...
## Changing some numeric variables into factors {.tabset}
...
## Visualization of important variables {.tabset}
...
```{r message=FALSE, warning=FALSE, warning=FALSE, message=FALSE, out.width="100%"}
...
## Preparing data for modeling {.tabset}
...
## Compare different regression models {.tabset}
...
## Kaggle submission 
  ...
  - Score: [0.119](https://www.kaggle.com/c/house-prices-advanced-regression-techniques/leaderboard#score)
  - No. of submissions: 3
  - Username: [salmaeng](https://www.kaggle.com/salmaeng)
